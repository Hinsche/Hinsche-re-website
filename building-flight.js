...
(function () {
  ...
  class BuildingFlight extends HTMLElement {
    ...
    async _boot() {
      ...
      renderer.setPixelRatio(Math.min(devicePixelRatio || 1, innerWidth < 780 ? 1.4 : 1.75));
      ...
      const sections = Array.from(this.children).filter((el) => el.tagName === 'SECTION');
      let shown = 0, target = 0;
      // stage index from the sections' own centres, so unequal section heights
      // (mobile: taller than the viewport) still line up with their keyframe
      const readScroll = () => {
        const forced = this.getAttribute('data-p');
        if (forced !== null) { target = clamp(parseFloat(forced) || 0, 0, 1); return; }
        const top = this.getBoundingClientRect().top;
        const vc = -top + innerHeight / 2;
        const c = sections.map((s) => s.offsetTop + s.offsetHeight / 2);
        let idx;
        if (!c.length) idx = 0;
        else if (vc <= c[0]) idx = 0;
        else if (vc >= c[c.length - 1]) idx = c.length - 1;
        else {
          let k = 0;
          while (k < c.length - 2 && vc > c[k + 1]) k++;
          idx = k + (vc - c[k]) / Math.max(c[k + 1] - c[k], 1);
        }
        target = clamp(idx / Math.max(sections.length - 1, 1), 0, 1);
      };
      ...
      const resize = () => {
        ...
        camera.aspect = w / h;
        // portrait screens need a wider lens or the interior reads as a slot
        camera.fov = camera.aspect < 0.85 ? 66 : camera.aspect < 1.2 ? 56 : 46;
      ...
      this._dbg = { camera, scene, renderer, group, wire, materials, lineMat, apply: () => shown, target: () => target };
      const tick = () => {
